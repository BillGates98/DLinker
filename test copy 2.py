...
def cosine_sim(v1=[], v2=[]):
    ...
    output = cosine
    # output = cosine # sigmoid(dot) # max
    # print(output)
    # doremus <= 0.1
    # anatomy <= 0.5
    # SPIMBENCH_small-2016 <= 0.1 
    # SPIMBENCH_small-2019 <= 0.1
    # UOBM_small-2016 <= 0.1
    # print('Dot product : ', dot, ' activated : ', output)
    # output = round(output, 2)
    if output >= 0.70 and output <= 0.80 :
        if not round(output,2) in probabilities :
            probabilities[round(output,2)] = 0
        probabilities[round(output,2)] += 1

    if output >= 0.70 and output <= 0.80  : # +0.05
        # print(output, ' rounded value : ' ,  round(output, 3))
        return True
    return False
...
def random_choice(value='', data=[], n = 500):
    ...
    values = [ ds.jaro_similarity(value1=value, value2=d)  for d in _data ] # jaro_similarity
    score = max(values)
...
def process_rdf_files(file1, file2):
    ...
    vectors1 = extract_entity_vectors_rdf(graph=_graph1, index=1)
    vectors2 = extract_entity_vectors_rdf(graph=_graph2, index=2)

    rand_count = 500 # int(( _g1_length if  _g1_length >= _g2_length else _g2_length ) * 0.09)
    ...
    for s2, p2, o2 in tqdm(_graph2) :
        ...
        if not validators.url(_o2) :
            ...
            if score >= 0.8 :
                ...
                for s1 in sub_s1:
                    _s1 = str(s1)
                    subpair = _s1 + "@" + _s2 if  _g1_length >= _g2_length else _s2 + "@" + _s1 
